...
function renderList(){
  ...
  $('sub-list').innerHTML=shown.length?shown.map(r=>{const w=r.workshop||{};const when=r.received?new Date(r.received).toLocaleString():'';return `<div class="sub-card ${esc(r.status)}"><div class="sub-head"><div><h3>${esc(w.name||'(no name)')}</h3><div class="sub-meta"><span class="badge ${esc(w.type)}">${w.type==='agency'?'Agency':'Non-agency'}</span><span>${esc(w.emirate||'')}</span><span>${esc(when)}</span>${w.duplicateReview?'<span class="edit-flag">possible duplicate — review</span>':''}</div></div><span class="sub-status ${esc(r.status)}">${esc(r.status)}</span></div><dl class="sub-fields"><dt>Address</dt><dd>${esc(w.address)||'—'}</dd><dt>Phone</dt><dd>${esc(w.phone)||'—'}</dd><dt>Hours</dt><dd>${esc(w.hours)||'—'}</dd><dt>Makes</dt><dd>${esc((w.makes||[]).join(', '))||'—'}</dd>${w.type==='nonagency'?`<dt>Insurers</dt><dd>${esc((w.insurers||[]).join(', '))||'—'}`:''}<dt>Notes</dt><dd>${esc(w.notes)||'—'}</dd></dl><div class="sub-actions">${r.status==='pending'?`<button data-act="approve" data-id="${esc(r.id)}">Approve</button>`:''}${r.status==='approved'?`<button data-act="publish" data-id="${esc(r.id)}">Publish</button>`:''}${r.status!=='rejected'&&r.status!=='published'?`<button data-act="reject" data-id="${esc(r.id)}">Reject</button>`:''}${r.status!=='published'?`<button data-act="edit" data-id="${esc(r.id)}">Edit</button>`:''}${r.status==='published'&&w.id?`<button class="del" data-unpublish-row="${esc(w.id)}">Remove from directory</button>`:''}</div></div>`}).join(''):'<div class="empty">Nothing here yet.</div>'
}

$('sub-list').addEventListener('click',async e=>{
  const rm=e.target.closest('button[data-unpublish-row]');
  if(rm){
    const wid=rm.dataset.unpublishRow;
    if(!confirm('Remove this workshop from the live directory?\n\nVisitors will no longer see it, and its queue record is removed too. This cannot be undone.'))return;
    try{await api({action:'delete-published',id:wid},true);await load();if(verificationVisible)await loadVerification()}catch(err){alert(err.message)}
    return;
  }
  const b=e.target.closest('button[data-act]');if(!b)return;const id=b.dataset.id,act=b.dataset.act;try{if(act==='edit')return await editSubmission(id);if(act==='approve')await api({action:'approve',id},true);else if(act==='reject')await api({action:'reject',id},true);else if(act==='publish')await api({action:'publish',id},true);await load()}catch(err){alert(err.message)}});
...
$('verification-list').addEventListener('click',async e=>{
  const rm=e.target.closest('button[data-unpublish]');
  if(rm){
    const id=rm.dataset.unpublish;
    const w=verificationRows.find(x=>x.id===id);
    if(!confirm(`Remove "${w&&w.name?w.name:id}" from the live directory?\n\nVisitors will no longer see it. This cannot be undone.`))return;
    try{await api({action:'delete-published',id},true);await loadVerification();await load()}catch(err){alert(err.message)}
    return;
  }
  const b=e.target.closest('button[data-verify-status]');if(!b)return;
  try{await api({action:'verification',id:b.dataset.id,status:b.dataset.verifyStatus},true);await loadVerification()}catch(err){alert(err.message)}
});
